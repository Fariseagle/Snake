 ...
 * modified on Fri Jun  5 16:19:42 2009 by eroberts
 * further modified Sep 2012 by cschatz
 * -----------------------------------------------------
...
// Some "extra" prototypes from inside lpclib.h needed here
...
inline void Vector<ElemType>::checkRange(int index, const char *verb) {
	if (index < 0 || index >= size()) {
		Error("Attempt to " + string(verb) + " index "
		      + IntToString(index) + " in a vector of size "
		      + IntToString(size()) + ".");
	}
}
